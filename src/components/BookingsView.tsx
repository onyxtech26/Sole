...
import { Search, ChevronUp, ChevronDown, Edit, Trash2, ShieldAlert, Plus, X, Calendar } from 'lucide-react';
...
export default function BookingsView({
  ...
  const renderSortArrow = (col: SortColumn) => {
    ...
    return sortDirection === 'asc' 
      ? <ChevronUp className="w-3.5 h-3.5 text-indigo-500 inline-block ml-0.5" />
      : <ChevronDown className="w-3.5 h-3.5 text-indigo-500 inline-block ml-0.5" />;
  };
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white/40 border border-slate-200/80 p-5 rounded-2xl backdrop-blur-xl shadow-xl shadow-black/[0.01]">
        <div className="relative w-full md:max-w-xs">
          <input
            ...
            className="w-full bg-white/50 border border-slate-200/80 rounded-xl pl-10 pr-4 py-3 text-slate-800 text-sm font-semibold outline-none transition duration-300 focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10"
          />
        ...
        <div className="flex flex-wrap gap-3 w-full md:w-auto items-center">
          <select
            ...
            className="bg-white/50 border border-slate-200/80 rounded-xl px-4 py-3 text-slate-800 text-sm font-semibold outline-none cursor-pointer focus:border-indigo-500 max-w-xs truncate"
          >
          ...
          <select
            ...
            className="bg-white/50 border border-slate-200/80 rounded-xl px-4 py-3 text-slate-800 text-sm font-semibold outline-none cursor-pointer focus:border-indigo-500"
          >
          ...
          return (
            ...
            >
              ...
              <span className={`ml-2 px-2 py-0.5 text-[10px] rounded-full font-bold transition-all ${
                isActive ? 'bg-indigo-50 text-indigo-600 border border-indigo-100' : 'bg-slate-100 text-slate-500'
              }`}>{count}</span>
              {isActive && (
                <motion.div 
                  layoutId="activeTabUnderline"
                  className="absolute bottom-0 left-0 w-full h-[2.5px] bg-indigo-600 rounded-t-full shadow-lg shadow-indigo-600/30" 
                />
              )}
            </button>
          );
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredBookings.length === 0 ? (
                <tr>
                  <td colSpan={currentUser.role === 'staff' ? 8 : 9} className="p-12 text-center">
                ...
                filteredBookings.map(b => (
                  <tr key={b.bookingRef} className="hover:bg-slate-50/50 transition">
                    <td className="p-4 text-center font-mono font-extrabold text-sm text-indigo-600 whitespace-nowrap">
                      {b.bookingRef}
                    </td>
                    ...
                    <td className="p-4 max-w-[200px]">
                      ...
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {b.assignedGuide && (
                          <span className="inline-flex items-center px-2 py-0.5 rounded-lg border border-indigo-100 bg-indigo-50 text-[9px] font-bold text-indigo-600">
                            👤 {b.assignedGuide}
                          </span>
                        )}
                        {b.assignedVehicle && b.assignedVehicle !== 'None' && (
                          <span className="inline-flex items-center px-2 py-0.5 rounded-lg border border-emerald-100 bg-emerald-50 text-[9px] font-bold text-emerald-600">
                            🚘 {b.assignedVehicle.split(' ')[0]}
                          </span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="p-4 text-center whitespace-nowrap">
                      <div className="flex flex-col gap-1.5 w-[115px] mx-auto">
                        <select
                          ...
                          className={`w-full text-center text-[10px] font-extrabold uppercase tracking-wide border px-2.5 py-1.5 rounded-full bg-white/80 cursor-pointer focus:outline-none focus:ring-4 focus:ring-indigo-500/10 ${
                            b.status === 'Confirmed' ? 'border-emerald-200 text-emerald-600 bg-emerald-50/50' :
                            b.status === 'Pending' ? 'border-indigo-200 text-indigo-600 bg-indigo-50/50' :
                        ...
                        >
                          ...
                          <option value="Pending" className="bg-white text-indigo-600 font-bold">Pending</option>
                        ...
                        <select
                          ...
                          className={`w-full text-center text-[10px] font-extrabold uppercase tracking-wide border px-2.5 py-1.5 rounded-full bg-white/80 cursor-pointer focus:outline-none focus:ring-4 focus:ring-indigo-500/10 ${
                            ...
                            b.paymentStatus === 'Partially Paid' ? 'border-indigo-200 text-indigo-600 bg-indigo-50/50' :
                        ...
                        >
                          ...
                          <option value="Partially Paid" className="bg-white text-indigo-600 font-bold">Partially Paid</option>
                    ...
                    <td className="p-4 pr-6 text-center whitespace-nowrap shrink-0">
                      <div className="flex gap-2 justify-center">
                        <button
                          ...
                          title="Edit Booking"
                          className="p-2 rounded-xl bg-slate-50 border border-slate-200/80 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 hover:border-indigo-100 transition duration-200 cursor-pointer transform hover:scale-105"
                        >
          ...
          {showFormModal && (
            ...
            >
              ...
              >
                ...
                <div className="w-10 h-10 rounded-full bg-indigo-50 border border-indigo-100 flex items-center justify-center mb-4 shrink-0">
            ...
            >
              ...
              >
                ...
                <p className="text-slate-500 text-xs font-semibold leading-relaxed mb-5">
                  Are you sure you want to delete reservation <span className="font-mono text-indigo-600 font-extrabold">{deleteConfirmRef}</span>? This action is permanent and cannot be undone.
                </p>
